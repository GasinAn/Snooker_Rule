...
\noindent 被在本规则中通篇使用的标准术语是被意大利体化的\footnote{意大利体是一种斜体。为编纂方便，仅在本节中意大利体化标准术语。}。\\
Standard definitions used throughout these Rules are italicised.

\subsection{局}\label{2221}

\noindent 斯诺克的一\textit{局}由某比赛时段组成，此比赛时段从开始(\see{}第\ref{223}节规则\ref{2233}\ref{2233c})起，每位球员[依次]在[自己的]\textit{击球轮}中[上台]比赛直到此\textit{局}结束并且是由于：\\
...
\begin{enumerate}[label=(\alph*)]
    \item 自任意[一方]球员的被[对方]接受的认输；或\\
    an accepted concession by any player; or
    \item 当黑球是台面上仅剩的\textit{目标球}, 累计得分[与比赛结果]无关, 并且[双方的]分数间有多于七分的差距且\textit{击球手}[方]领先时，自\textit{击球手}的[己方赢得此局的]申明；或\\
    ...
    \item 当黑球是台面上仅剩的\textit{目标球}时，首次\textit{击球入袋}或\textit{犯规}(\see{}第\ref{223}节规则\ref{2234})；或\\
...
\noindent 即将[上台]比赛或[上台]比赛中的人是\textit{击球手}。他们的\textit{击球轮}存续直到：\\
...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item 任何被\textit{击球手}使用的辅助器材都已被拿走，或裁判认可此次\textit{击球}已结束。\\
    ...
    \begin{enumerate}[label=(\roman*)]
        \item 当\textit{母球}没有首先击中库边而击中一颗\textit{目标球}时\textit{击球}是\textit{直接的}；\\
        a \textit{stroke} is \textit{direct} when the \textit{cue-ball} hits an \textit{object ball} without first hitting a cushion;
        \item 当\textit{母球}在击中一颗\textit{目标球}前击中一个或多个库边时\textit{击球}是\textit{间接的}。\\
...
\noindent 一次\textit{入袋}是一颗\textit{目标球}, 与另一颗球接触后且在无任何本规则中的\textit{违规}伴随的情况下, 进入袋中的时候。造成一颗球被\textit{击球入袋}被称为\textit{击球入袋}。在一次\textit{犯规}\textit{击球}中造成一颗球进入球袋被称为\textit{掉袋}。\\
...
\noindent 一次\textit{单杆}是由\textit{击球手}在任意一个\textit{击球轮}中进行的连续\textit{击球}之中的若干\textit{入袋}。\\
...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item 当黑球如第\ref{223}节规则\ref{2234}\ref{2234b}中[所述]地被\respotted{}时。\\
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item 自本\textit{局}的开始起直到被\textit{击球入袋}、\textit{掉袋}或被\textit{迫离台面}\textit{目标球}[都]处于\textit{比赛中状态}。\\
    ...
    \item 当被\respotted{}时彩球重新变成处于\textit{比赛中状态}。\\
...
\begin{enumerate}[label=(\alph*)]
    \item \textit{指定球}是\textit{击球手}合裁判的认可地\indicate{}的，或\declare{}(口头声明)的，他们承诺要通过\textit{母球}的首次撞击击中的\textit{目标球}。\\
    ...
    \item 如果被裁判要求，\textit{击球手}必须\declare{}他们认\textit{活}哪颗球。\\
    If requested by the referee, the \textit{striker} must declare which ball they are \textit{on}.
\end{enumerate}

\subsection{自由球}

\noindent \textit{自由球}\footnote{不要认成八球的自由球！！！译者自己常直接称为“飞波”以避免误认。}是当[\textit{母球}]在一次\textit{犯规}后被\textit{做斯诺克}(\see{}第\ref{223}节规则\ref{22312})时\textit{击球手}\textit{指定}当成\textit{活球}的不是\textit{活球}的球。\\
...
\noindent 如果一颗球变得静止但不在\playingarea{}上或不在球袋中那么它被\textit{迫离台面}。\\
A ball is \textit{forced off the table} if it comes to rest other than on the 
playing area or in a pocket.

\subsection{违规、犯规和\termpenalty{}}

\noindent \textit{违规}是任意对本规则的违背。\\
An \textit{infringement} is any violation of these Rules.\\
\textit{犯规}是会结束违规方\textit{击球轮}的\textit{违规}。\\
A \textit{foul} is an \textit{infringement} which will end the offender's \textit{turn}.\\
\textit{\termpenalty{}}是不影响比赛次序的\textit{违规}。\\
\textit{Penalties} are \textit{infringements} which do not affect the order of play.
...
\noindent 当对每颗\textit{活球}的沿一条直线的\textit{直接击球}都被一颗或多颗非\textit{活}球完全或部分阻挡时\textit{母球}被\textit{做斯诺克}。如果一颗或更多颗\textit{活球}不受任何非\textit{活}球的阻挡影响而两个薄边都能被击中，那么\textit{母球}没有被\textit{做斯诺克}。\\
...
\begin{enumerate}[label=(\alph*)]
    \item 如果[\textit{母球}处于]\textit{手中球状态}，那么若\textit{母球}在D形区边线上或边线内的任何可能[被摆放的]位置都被如上面所述地阻挡则它被\textit{做斯诺克}。\\
    ...
    \item 当\textit{母球}被如上面[所述]地\textit{做斯诺克}时\textit{击球手}[也]被称为被\textit{做斯诺克}。\\
...
\noindent 一次\textit{跳球}被造成于某时候，此时候\textit{母球}越过一颗\textit{目标球}的任意部分，且无论在此过程中是否将其击中，除非：\\
...
\begin{enumerate}[label=(\alph*)]
    \item 此时候\textit{母球}首先击中一颗, 不是[与\textit{母球}]相贴的球的, \textit{目标球}，并然后跳起越过另一颗球；或\\
    when the \textit{cue-ball} first hits one \textit{object ball}, other than a touching ball, and then jumps over another ball; or
    \item 此时候\textit{母球}跳起并击中一颗, 不是[与\textit{母球}]相贴的球的, \textit{目标球}，并且在着陆到\playingarea{}上的时刻，\textit{母球}不在此\textit{目标球}的当前位置的远端；或\\
    ...
    \item 此时候, 在合法地击中一颗, 不是[与\textit{母球}]相贴的球的, \textit{目标球}后，\textit{母球}击中库边或另一颗球后再跳起越过此[\textit{目标}]\textit{球}。\\
...
\noindent 一次\textit{\miss{}}\footnote{也有人译为“未尽力”。}是：\\
A \textit{miss} is:
\begin{enumerate}[label=(\alph*)]
    \item \textit{母球}未能首先接触一颗\textit{活球}的时候；或\\
    when the \textit{cue-ball} fails to first contact a \textit{ball on}; or
    \item 当一颗\textit{自由球}已被\textit{指定}时，\textit{母球}既未能首先击中彼颗\textit{被指定的}\textit{自由球}也未能首先同时击中彼颗球和一颗\textit{活球}的时候。\\
...
\noindent \textit{磋商时间}是球员可以就将任意球摆回到\textit{违规}被造成(第\ref{223}节规则\ref{2232}\ref{2232c}\ref{2232cii}、\ref{2233}\ref{2233k}、\ref{22310}\ref{22310i}、\ref{22314}、和\ref{22316})前的或如第\ref{223}节规则\ref{2239}和\ref{22315}中所述的原来的位置对裁判给予帮助的时间。\textit{磋商时间}在将球摆回的决定被做出的时刻开始，并在双方球员都对球的位置表示认可时结束或依裁判的最终决定结束。\\
A \textit{consultation period} is the period in which players may assist the referee with replacing any ball(s) to the original position prior to when an \textit{infringement} was committed (Section \ref{223} Rules \ref{2232}\ref{2232c}\ref{2232cii}, \ref{2233}\ref{2233k}, \ref{22310}\ref{22310i}, \ref{22314}, and \ref{22316}) or as described in \ref{223} Rule \ref{2239} and \ref{22315}. The \textit{consultation period} starts from the moment the decision is made to replace the ball(s) and ends when both players are satisfied as to the position of the ball(s), or by the referee's final decision.
